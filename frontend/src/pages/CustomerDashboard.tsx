...
const CustomerDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState("marketplace"); // Add tab state
  ...
  const fetchUserInvestments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Fetch portfolio summary from dedicated endpoint
  ...
  const fetchPortfolioSummary = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/users/portfolioSummary`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Portfolio summary API response:', data);

        const summaryData = data.data || data;
        setPortfolioSummary(summaryData);
      } else {
        console.log('Portfolio summary API failed');
      }
    } catch (error) {
  ...
  const handlePurchase = async (productId: string, amount: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refresh user investments and portfolio summary
      ...
      // Refresh portfolio summary from API instead of manual calculation
      await fetchPortfolioSummary();
    }
  };
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                ...
                <TabsContent value="insights">
                  <InsightsSection 
                    ...
                    onSwitchToRecommendations={() => setActiveTab("recommendations")}
                  />
                </TabsContent>
              </Tabs>
            </div>
