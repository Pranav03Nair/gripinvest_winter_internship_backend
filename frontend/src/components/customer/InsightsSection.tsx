...
interface InsightsData {
  ...
  insights: string; // Changed from string[] to string since API returns a single string
}
...
export const InsightsSection: React.FC<InsightsSectionProps> = ({ 
  ...
  const fetchInsights = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/investments/portfolioInsights`, {
        headers: {
      ...
      if (response.ok) {
        ...
        // Check if user has investments/insights data
        if (!insightsData || !insightsData.totalInvested || insightsData.totalInvested === 0) {
          console.log('No investments found for user, showing fallback message');
          setInsights(null);
          setError('no-investments');
          return;
        }
      ...
      const mockInsights: InsightsData = {
        ...
        insights: "Your portfolio has a high-risk allocation of 59.46%, which may lead to higher volatility but potentially greater returns. Consider diversifying into more low-risk investments to balance your portfolio and reduce overall risk. Your expected returns of ₹12,500 represent a 16.67% return on your total investment."
      };
  ...
  if (error) {
    // Special case for new users with no investments
    if (error === 'no-investments') {
      return (
        <div className="space-y-6">
          <Card className="financial-card">
            <CardHeader>
            ...
            <CardContent>
              <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center">
                  <ChartPie size={32} className="text-primary" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-semibold">No Insights Available Yet</h3>
                  <p className="text-muted-foreground max-w-md">
                    Buy investments to generate insights. Checkout our Recommendations for curated options.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 mt-6">
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      if (onSwitchToMarketplace) {
                        ...
                        const marketplaceTab = document.querySelector('[value="marketplace"]');
                        if (marketplaceTab) {
                          (marketplaceTab as HTMLElement).click();
                        }
                      }
                    }}
                  ...
                  <motion.button
                    ...
                    className="px-6 py-2 border border-border rounded-lg font-medium hover:bg-secondary/50 transition-colors"
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {/* Updated to handle string insights by splitting into paragraphs */}
