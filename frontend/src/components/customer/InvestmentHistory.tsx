...
export const InvestmentHistory: React.FC<InvestmentHistoryProps> = ({
  ...
  const handleCancelInvestment = async (investmentId: string) => {
    try {
      ...
      const refundAmount = investment ? Number(investment.amount || 0) : 0;
              ...
              <Table>
                ...
                <TableBody>
                  {investments.map((investment, index) => {
                    try {
                      // Safe data access with fallbacks - use correct schema property names
                      const amountInvested = Number(investment.amount || 0);
                      const currentValue = Number(investment.amount || 0); // Use amount since current_value doesn't exist in DB
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>{formatDate(investment.invested_at || new Date().toISOString())}</TableCell>
