                ...
                >
                  <Card className="h-full hover:shadow-md transition-all duration-300 cursor-pointer group border border-border">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <Button
                        onClick={() => {
                          // Add validation before investing
                          if (!user) return;
                          
                          const amount = product.min_investment;
                          
                          // Check balance
                          if (amount > (user.balance || 0)) {
                            toast({
                              title: "Insufficient Balance",
                              description: `You need ${formatCurrency(amount)} but only have ${formatCurrency(user.balance || 0)}. Please add funds to your account.`,
                              variant: "destructive",
                            });
                            return;
                          }
                          
                          // Check minimum amount (already using min_investment, so this should pass)
                          if (amount < product.min_investment) {
                            toast({
                              title: "Amount Too Low",
                              description: `Minimum investment amount is ${formatCurrency(product.min_investment)}.`,
                              variant: "destructive",
                            });
                            return;
                          }
                          ...
                          if (product.max_investment && amount > product.max_investment) {
                            toast({
                              ...
                              description: `Maximum investment amount is ${formatCurrency(product.max_investment)}.`,
                              variant: "destructive",
                            });
                            return;
                          }
                          
                          onInvest?.(product.id, amount);
                        }}
