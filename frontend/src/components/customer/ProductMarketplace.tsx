...
export const ProductMarketplace: React.FC<ProductMarketplaceProps> = ({
  ...
  const handleInvest = () => {
    if (!selectedProduct || !investmentAmount || !user) return;

    // Validation checks
    if (investmentAmount > (user.balance || 0)) {
      toast({
        title: "Insufficient Balance",
        description: `You need ${formatCurrency(investmentAmount)} but only have ${formatCurrency(user.balance || 0)}. Please add funds to your account.`,
        variant: "destructive",
      });
      return;
    }
    ...
    if (investmentAmount < selectedProduct.min_investment) {
      toast({
        ...
        description: `Minimum investment amount is ${formatCurrency(selectedProduct.min_investment)}.`,
        variant: "destructive",
      });
      return;
    }

    if (selectedProduct.max_investment && investmentAmount > selectedProduct.max_investment) {
      toast({
        title: "Amount Too High",
        description: `Maximum investment amount is ${formatCurrency(selectedProduct.max_investment)}.`,
        variant: "destructive",
      });
      return;
    }
