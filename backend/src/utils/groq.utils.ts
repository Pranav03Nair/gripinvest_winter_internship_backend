...
export async function generateProductRecommendations(
  ...
  const productList = products
    .map(
      (p, index) =>
        `${index + 1}. Name: ${p.name}, Type: ${p.investment_type}, Tenure: ${p.tenure_months} months, Yield: ${p.annual_yield}%, Risk: ${p.risk_level}, Min: ₹${p.min_investment}${p.max_investment ? `, Max: ₹${p.max_investment}` : ''}`,
    )
    .join('\n');
...
export async function generatePortfolioInsights(
  investments: InvestmentWithProducts[],
  ...
  const investmentSummary = investments
    .map(
      (inv) =>
        `${inv.investment_products.name}: ₹${inv.amount} in ${inv.investment_products.risk_level} risk, yield ${inv.investment_products.annual_yield}%`,
    )
    .join('\n');
...
export async function generateErrorSummary(
