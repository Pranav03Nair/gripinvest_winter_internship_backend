...
describe('Auth Service', () => {
    ...
    const mockSignToken = jwt_util_1.signToken;
    beforeEach(() => {
        jest.clearAllMocks();
    });
    describe('createUser', () => {
        ...
        it('should throw error if user already exists', async () => {
            ...
            await expect(authService.createUser(signupData)).rejects.toThrow('User wih this email already exists');
            expect(mockPrisma.users.findUnique).toHaveBeenCalledWith({
                where: { email: signupData.email },
            });
    ...
    describe('loginUser', () => {
        ...
        it('should throw error if user does not exist', async () => {
            mockPrisma.users.findUnique.mockResolvedValue(null);
            await expect(authService.loginUser(loginData)).rejects.toThrow("Invalid Credentials: User with this email, doesn't exist");
            expect(mockPrisma.users.findUnique).toHaveBeenCalledWith({
                where: { email: loginData.email },
            });
        ...
        it('should throw error if password is incorrect', async () => {
            ...
            await expect(authService.loginUser(loginData)).rejects.toThrow('Incorrect Password');
            expect(mockBcrypt.compare).toHaveBeenCalledWith(loginData.password, mockUser.password_hash);
