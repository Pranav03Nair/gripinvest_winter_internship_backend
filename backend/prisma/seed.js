...
async function main() {
    ...
    if (existingAdmin) {
        console.log(`Admin exists already. Credentials:\n Email - ${adminMail}\n Pass - ${adminPass}`);
    }
    if (!existingAdmin) {
        const password_hash = await bcrypt_1.default.hash(adminPass, 10);
    ...
    for (const user of users) {
        ...
        if (existingUser) {
            console.log(`User exists already. Credentials:\n Email - ${user.email}\n Pass - ${user.password}`);
        }
        if (!existingUser) {
            const password_hash = await bcrypt_1.default.hash(user.password, 10);
            await prisma.users.create({
                data: {
